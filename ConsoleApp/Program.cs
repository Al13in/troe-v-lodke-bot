...
foreach (var taskType in taskTypes)
{
    ...
    while (true)
    {
        try
        {
            ...
            if (!ok)
            {
                Console.WriteLine($"ОСТАНОВКА: неверный ответ на задачу типа {taskType}. Исправьте солвер и запустите снова.");
                globalStop = true;
                break;
            }
            errorsInARow = 0;
        ...
        catch (Exception ex)
        {
            ...
            {
                Console.WriteLine($"Задачи {taskType} исчерпаны (400). ErrorName={apiEx.ErrorName}, Description={apiEx.ErrorDescription}. OK={successCount} FAIL={failCount}");
                break;
            }
